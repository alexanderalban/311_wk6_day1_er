...
The actor table has a primary key of actor_id. This primary key is also present in the film_actor table. This can be confirmed when opening the "index" below the table and hovering over the "PRIMARY" entry.

2. What does the blue diamond next to the "last_update" column on the "inventory" table represent?

The blue diamond is filled in, so this represents a simple attribute that cannot be a null ("NOT NULL").

3. How many foreign keys does the "payments" table have? How can you tell?

The payment table hsa three foreign keys. You can tell due to the red diamonds next to the entries: customer_id and staff_id have filled in diamonds (meaning that they cannot be NULL), and rental_id is not filled in, meaning it can have a NULL value.
